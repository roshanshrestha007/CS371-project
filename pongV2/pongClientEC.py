...
def watchGame(screenWidth: int, screenHeight: int, playerPaddle: str, client: socket.socket) -> None:
    # Pygame inits
    pygame.mixer.pre_init(44100, -16, 2, 2048)
    pygame.init()
    ...
    while True:
        ...
        while True:
            ...
            if cmd['command'] == 'UPDATE_TIME':
                lScore = cmd['score'][0]
                rScore = cmd['score'][1]
                ball.rect.x = cmd['ball'][0]
                ball.rect.y = cmd['ball'][1]
                leftPaddle.rect.y = cmd['left_y']
                rightPaddle.rect.y = cmd['right_y']
                # todo: ack
                acknowledge(client)
            elif cmd['command'] == 'leftwin':
                textSurface = winFont.render("Player 1 Won", False, WHITE, (0, 0, 0))
                textRect = textSurface.get_rect()
                textRect.center = ((screenWidth / 2), screenHeight / 2)
                screen.blit(textSurface, textRect)
                ...
                while True:
                    ...
                    clock.tick(60)
                    # break#todo
            elif cmd['command'] == 'rightwin':
                textSurface = winFont.render("Player 2 Won", False, WHITE, (0, 0, 0))
                textRect = textSurface.get_rect()
                textRect.center = ((screenWidth / 2), screenHeight / 2)
                screen.blit(textSurface, textRect)
                while True:
...
def joinServer(ipEntry: str, portEntry: str, errorLabel: tk.Label, app: tk.Tk) -> None:
    ...
    if side == "s":
        watchGame(screenWidth, screenHeight, side, client)
    else:
...
def startScreen():
    ...
    app.after(100, check_game_started, app, joinButton, waiting_label)
    app.mainloop()


def check_game_started(app, joinButton, waiting_label) -> None:
